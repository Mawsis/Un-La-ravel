...
| [[ADR 0012 - Middleware as a node\|0012]] | Middleware is a node; reverse index is derived, not serialized | ✅ accepted | The last MVP node type gains a node (`middlewares`, contract `1.10.0`); "which routes apply this" is a read-time join over the routes, never a stored edge, so it can't drift; direct vs group-transitive stay distinct labeled relations |
